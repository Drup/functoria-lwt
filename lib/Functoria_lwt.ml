...
let tool_name = "functoria-lwt"

let src = Logs.Src.create tool_name ~doc:"functoria-lwt cli tool"
...
|}
  ...
  let packages = [package "functoria-lwt"]
  ...
  let create jobs = impl @@ object
      ...
      method module_name = "Functoria_lwt_runtime"
      method! keys = [
      ]
      method! packages =
        let common = [
